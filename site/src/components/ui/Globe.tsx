...
interface LandDot { lat: number; lng: number; }
interface Arc { startLat: number; startLng: number; endLat: number; endLng: number; }
interface Label { lat: number; lng: number; }
...
export const Globe: React.FC<GlobeProps> = ({
      ...
      return new Promise((resolve, reject) => {
        ...
        if (existing) {
          ...
          setTimeout(() => {
            ...
            if (window.Globe) { resolve(); } else { reject(new Error("Globe library loaded but not available")); }
          }, 5000);
          return;
        }
  ...
  const processLandMap = useCallback((image: HTMLImageElement): LandDot[] => {
    ...
    for (let lat = -90; lat <= 90; lat += 180 / landDotRows) {
      ...
      for (let r = 0; r < circum; r++) {
        ...
        if (visibilityForCoordinate(lng, lat)) dots.push({ lat, lng });
      }
    }
    ...
    const initGlobeDeferred = () => {
      ...
      landMapImage.onload = () => {
        ...
        const world = window.Globe()
          ...
          .pointRadius(pointSize).pointResolution(pointResolution)
          .pointAltitude(0).pointsMerge(true)
          .arcColor(() => deferredPrimaryColor)
          .arcStroke(0.25).arcDashInitialGap(1).arcDashLength(2)
          .arcDashGap(2).arcDashAnimateTime(arcAnimationDuration)
          .labelText(() => "").labelColor(() => deferredPrimaryColor)
          .labelDotRadius(0.3).labelAltitude(0.002)
          .labelsTransitionDuration(250)
          .ringColor(() => (t: number) => `rgba(59, 130, 246, ${1 - t})`)
          .ringMaxRadius(2).ringPropagationSpeed(2).ringRepeatPeriod(0)(container);
        ...
        world.onGlobeClick((coords, event) => { if (onGlobeClickRef.current) onGlobeClickRef.current(coords, event); });
        globeRef.current = world;
        requestAnimationFrame(() => { requestAnimationFrame(() => { setIsGlobeVisible(true); }); });
        const animateArcs = () => {
          if (!globeRef.current || dotsRef.current.length === 0 || isAnimatingRef.current) return;
          if (!isVisibleRef.current) return;
          isAnimatingRef.current = true;
          animationFrameRef.current = requestAnimationFrame(() => {
            if (!globeRef.current || dotsRef.current.length === 0) { isAnimatingRef.current = false; return; }
            ...
            const arcs: Arc[] = Array.from({ length: arcCount }, (_, i) => ({
              startLat: selectedDots[i].lat, startLng: selectedDots[i].lng,
              endLat: selectedDots[i + arcCount].lat, endLng: selectedDots[i + arcCount].lng,
            }));
            const labels: Label[] = Array.from({ length: arcCount }, (_, i) => ({
              lat: selectedDots[i + arcCount].lat, lng: selectedDots[i + arcCount].lng,
            }));
        ...
        if ("IntersectionObserver" in window) {
          observer = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
              ...
              if (globeRef.current) globeRef.current.controls().autoRotate = entry.isIntersecting;
            });
          }, { threshold: 0.1 });
      ...
      landMapImage.onerror = () => { setError("Failed to load land map image"); isInitializingRef.current = false; };
    };
    if ("requestIdleCallback" in window) { requestIdleCallback(initGlobeDeferred, { timeout: 500 }); }
    else { setTimeout(initGlobeDeferred, 0); }
    return () => { if (cleanupFnRef.current) { cleanupFnRef.current(); cleanupFnRef.current = null; } };
  }, [isLoading, error, width, height, deferredPrimaryColor, deferredNeutralColor, deferredAtmosphereColor, deferredGlobeColor, showAtmosphere, autoRotateSpeed, enableZoom, interactive, arcCount, arcInterval, arcAnimationDuration, cameraAltitude, landMapUrl, processLandMap, onReady, cleanup, pointSize, pointResolution, atmosphereAltitude, globeOpacity]);

  if (error) {
    return (
      <div className={cn("flex items-center justify-center rounded-lg border border-red-200 bg-red-50 p-8 text-red-600", className)}>
